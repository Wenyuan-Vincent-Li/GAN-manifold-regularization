...
class GAN_manifold_reg():
    ...
    def classifier(self, inp, is_training, init=False, reuse=False, getter=None):
        with tf.variable_scope('classifier', reuse=reuse, custom_getter=getter):
            ...
            x = nn.conv2d(x, 256, nonlinearity = self.leakyReLu, init=init, counters=counter)
            x = nn.conv2d(x, 256, nonlinearity = self.leakyReLu, init=init, counters=counter)
            x = nn.conv2d(x, 256, stride=[2, 2], nonlinearity = self.leakyReLu, init=init, counters=counter)

            x = tf.layers.dropout(x, rate=0.5, training = is_training, name='dropout_2')

            x = nn.conv2d(x, 512, pad='VALID', nonlinearity = self.leakyReLu, init=init, counters=counter)
            x = nn.nin(x, 256, counters=counter, nonlinearity = self.leakyReLu, init=init)
